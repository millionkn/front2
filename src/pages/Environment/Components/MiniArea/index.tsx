...
import autoHeight from '@/pages/DashboardAnalysis/components/Charts/autoHeight';
...
export interface MiniAreaProps {
  ...
  xAxis?: AxisProps;
  forceFit?: boolean;
  tooltipFormatter?: (x: string, y: string) => { name: string, value: string },
  scale?: {
    x?: {
      tickCount: number;
    };
...
const MiniArea: React.FC<MiniAreaProps> = (props) => {
  const {
    ...
    yAxis,
    animate = true,
    tooltipFormatter = (x: string, y: string) => ({
      name: x,
      value: y,
    }),
  } = props;
